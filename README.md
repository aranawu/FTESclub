...
- 報名完成後自動寄送收件通知；學生的所有社團選項皆審核完成後，系統只寄送一封星期三、星期五統整結果信。
- 家長不建立帳號，只在報名表提供電子郵件；管理端使用單一管理者帳號密碼登入。
- 班級由年級自動設定為一年忠班至六年忠班，後端會再次強制校正。
- 管理端可手動補登無法自行報名的學生（不需輸入身分證字號），也可填候補序號、匯出 CSV，並由後端生成個別學生通知單、每班一頁的班級彙整通知單，以及可逐社團選擇與列印的錄取名單。
...
2. 家長無法自行送出時，按「手動新增報名」，填寫姓名、年級與社團；電話、信箱可選填。未填信箱的學生請使用紙本通知單。
3. 每一筆報名（包含不限額社團）都須人工選擇錄取、候補或未錄取；候補者填候補序號。
4. 按「儲存審核結果」。若該生還有其他社團待審核，系統只儲存、不寄信；全部完成後才寄送一封週三、週五統整結果信。
5. 按「列印通知單」。通知單由已驗證身分的後端生成，可直接列印或另存 PDF。
