...
function printableDocument(registrations, schoolName, title) {
  ...
  </style></head><body><div class="actions"><button onclick="window.print()">列印／另存 PDF（共 ${registrations.length} 位）</button></div>${sheets}</body></html>`;
}

export function printableNotice(registration, schoolName) {
  return printableDocument([registration], schoolName, `${registration.studentName}－社團結果通知單`);
}

export function printableNotices(registrations, schoolName) {
  return printableDocument(registrations, schoolName, `課後社團個人通知單（${registrations.length} 位）`);
}
