...
function dayCard(registration, day) {
  const choice = registration.choices.find((item) => item.club.day === day);
  const dayName = `星期${day}`;
  const dayIcon = day === '三' ? '🌱' : '🌻';
  if (!choice) {
    return `<article class="day-card day-empty"><header><span class="day-pill">${dayIcon} ${dayName}</span><span class="result-pill empty">未報名</span></header><h2>本日未報名</h2><p class="empty-copy">這一天沒有選擇社團。</p></article>`;
  }
...
function noticeSheet(registration, schoolName) {
  return `<section class="sheet" data-print-sheet><header class="notice-hero"><p class="notice-kicker">🌾 ${escapeHtml(schoolName)}｜115學年度</p><h1>課後社團錄取結果通知單</h1><p>一起快樂學習，讓興趣在校園裡發芽！</p></header><p class="greeting">親愛的家長您好，以下為學生星期三與星期五的社團統整結果：</p><div class="student-meta"><div><span>學生姓名</span><strong>${escapeHtml(registration.studentName)}</strong></div><div><span>班級</span><strong>${escapeHtml(registration.className)}</strong></div><div><span>報名編號</span><strong>${escapeHtml(registration.registrationNo)}</strong></div></div><div class="club-day-grid">${dayCard(registration, '三')}${dayCard(registration, '五')}</div><footer class="notice-footer"><span>如對結果有疑問，請洽學校社團承辦人。</span><span>請妥善保存本通知單 🌼</span></footer></section>`;
}
