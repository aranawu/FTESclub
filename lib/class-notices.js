...
export function printableClassNotices(registrations, schoolName, logoUrl = '') {
  ...
  const sheets = groups.map((group, index) => {
    ...
    return `<section class="sheet" id="class-${index + 1}"><header>${logoUrl ? `<img src="${escapeHtml(logoUrl)}" alt="校徽">` : ''}<div><p>${escapeHtml(schoolName)}</p><h1>115學年度課後社團名單－${escapeHtml(group.grade)}${escapeHtml(group.className)}</h1></div></header><p class="intro">以下依審核結果分表列出，敬請導師協助核對並轉知學生及家長。</p><div class="summary"><div class="accepted"><span>錄取</span><strong>${accepted.length}</strong><small>人次</small></div><div class="attention"><span>候補／審核中</span><strong>${attention.length}</strong><small>人次</small></div><div class="rejected"><span>未錄取</span><strong>${rejected.length}</strong><small>人次</small></div></div>${resultBlocks}<footer><span>本班共 ${group.registrations.length} 位學生、${entries.length} 個報名選項</span><span>列印日期：${new Date().toLocaleDateString('zh-TW')}</span></footer></section>`;
  }).join('');
  ...
  return `<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>課後社團班級通知單</title><style>
    :root{color:#111;background:#eef1ea;font-family:'Microsoft JhengHei',sans-serif}*{box-sizing:border-box}body{margin:0}.tools{position:sticky;top:0;z-index:2;display:flex;gap:10px;align-items:center;justify-content:center;padding:12px;background:#315b2a;color:#fff;box-shadow:0 3px 12px #0002}.tools select,.tools button{font:inherit;border:0;border-radius:999px;padding:9px 14px}.tools button{background:#ffd54a;color:#3f4c1f;font-weight:800;cursor:pointer}.sheet{width:min(195mm,calc(100% - 32px));min-height:272mm;margin:18px auto;padding:11mm;background:#fff;border:1px solid #c9c9c9;box-shadow:0 12px 30px #0001;break-after:page}.sheet>header{position:relative;display:flex;align-items:center;justify-content:center;min-height:58px;text-align:center;border-bottom:0;padding-bottom:6px}.sheet>header img{position:absolute;left:0;width:52px;height:52px;object-fit:contain}.sheet>header p{margin:0;font-size:12px}.sheet>header h1{margin:2px 0;font-family:DFKai-SB,BiauKai,'Microsoft JhengHei',serif;font-size:24px;font-weight:700}.intro{text-align:center;margin:3px 0 8px;font-size:12px;color:#555}.summary{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin-bottom:10px}.summary div{display:flex;align-items:baseline;justify-content:center;gap:5px;padding:5px;border:1px solid #aaa}.summary span{font-weight:800}.summary strong{font-size:18px}.summary small{font-size:11px}.summary .accepted{background:#eef8ef}.summary .attention{background:#fff8dc}.summary .rejected{background:#fff0f0}.result-section{margin:10px 0;break-inside:avoid}.section-title{display:flex;align-items:flex-end;justify-content:space-between;border-bottom:2px solid #111;padding:3px 1px}.section-title h2{margin:0;font-size:16px}.section-title p{margin:0;font-size:11px;color:#555}.accepted .section-title h2{color:#205b35}.attention .section-title h2{color:#725500}.rejected .section-title h2{color:#813535}table{width:100%;border-collapse:collapse;table-layout:fixed;margin-top:0}th,td{border:1px solid #111;padding:6px 4px;text-align:center;vertical-align:middle;font-size:12px;height:29px}th{background:#f2f2f2;font-weight:800}th:first-child,td:first-child{width:7%}th:nth-child(2),td:nth-child(2){width:10%}th:nth-child(3),td:nth-child(3){width:12%}th:nth-child(4),td:nth-child(4){width:31%}th:nth-child(5),td:nth-child(5){width:17%}th:nth-child(6),td:nth-child(6){width:15%}.attention th:nth-child(4),.attention td:nth-child(4){width:26%}.attention th:last-child,.attention td:last-child{width:14%}.student,.club-name{font-weight:500}.result-cell{font-weight:800}.no-result{margin:0;border:1px solid #111;border-top:0;padding:7px 10px;color:#666;text-align:center;font-size:12px}footer{display:flex;justify-content:space-between;margin-top:10px;color:#555;font-size:11px}.empty-sheet{text-align:center;padding-top:80px}@media(max-width:700px){.tools{flex-wrap:wrap}.sheet{width:calc(100% - 12px);padding:12px;min-height:0}.sheet>header{padding-left:45px}.sheet>header img{width:40px;height:40px}.sheet>header h1{font-size:18px}.summary{grid-template-columns:1fr}.summary div{justify-content:flex-start}.result-section{overflow-x:auto}.section-title{align-items:flex-start;flex-direction:column}table{min-width:650px}th,td{font-size:11px;padding:5px 3px}}@media print{body{background:#fff}.tools{display:none}.sheet{width:auto;min-height:0;margin:0;padding:0;border:0;box-shadow:none}.sheet.filtered-out{display:none}.sheet:last-child{break-after:auto}.result-section{break-inside:avoid}thead{display:table-header-group}}@page{size:A4 portrait;margin:10mm}
