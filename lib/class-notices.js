...
export function printableClassNotices(registrations, schoolName, stylesheetUrl = '/print.css') {
  ...
  const options = groups.map((group, index) => `<option value="class-${index + 1}">${escapeHtml(group.className)}</option>`).join('');
  ...
  </style><link rel="stylesheet" href="${escapeHtml(stylesheetUrl)}"></head><body class="class-notice-page"><div class="tools"><label for="classFilter">顯示班級</label><select id="classFilter" data-sheet-filter><option value="all">全部班級</option>${options}</select><button type="button" data-print-button>列印／另存 PDF</button></div>${groups.length ? sheets : empty}</body></html>`;
}
