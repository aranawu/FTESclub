...
export function printableClubRosters(registrations, schoolName, stylesheetUrl = '/print.css') {
  ...
  const sheets = CLUBS.map((club, index) => {
    ...
    return `<section class="sheet club-sheet" id="club-${index + 1}" data-print-sheet><header><p>${escapeHtml(schoolName)}</p><h1>115學年度課後社團名單－${escapeHtml(club.name)}</h1><div class="club-meta"><span>星期${escapeHtml(club.day)}</span><span>${escapeHtml(club.time)}</span><span>${escapeHtml(club.period)}</span><span>${escapeHtml(club.location || '待公告')}</span></div></header><div class="roster-summary"><strong>錄取 ${members.length} 人</strong><span>${escapeHtml(capacity)}</span></div>${members.length ? `<table><thead><tr><th>序號</th><th>班級</th><th>姓名</th><th>上課時間</th><th>開始日期</th><th>地點</th></tr></thead><tbody>${rows}</tbody></table>` : '<p class="no-result">目前沒有錄取學生</p>'}<footer><span>本名單僅列已錄取學生</span><span>列印日期：${new Date().toLocaleDateString('zh-TW')}</span></footer></section>`;
  }).join('');

  return `<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>課後社團名單</title><link rel="stylesheet" href="${escapeHtml(stylesheetUrl)}"></head><body class="club-roster-page"><div class="tools"><label for="clubFilter">顯示社團</label><select id="clubFilter" data-sheet-filter><option value="all">全部社團</option>${options}</select><button type="button" data-print-button>列印／另存 PDF</button></div>${sheets}</body></html>`;
}
