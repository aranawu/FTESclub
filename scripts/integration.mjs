...
assert.ok(classPrintedHtml.includes('class="class-summary-table"'));
assert.ok(classPrintedHtml.includes('每位學生固定一列'));
assert.ok(classPrintedHtml.includes('審核結果及上課地點均彙整如下'));
assert.ok(classPrintedHtml.includes('錄取 <strong>'));
assert.ok(classPrintedHtml.includes('候補／審核中'));
assert.ok(classPrintedHtml.includes('未錄取 <strong>'));
assert.ok(classPrintedHtml.includes('<th rowspan="2">班級</th>'));
assert.ok(classPrintedHtml.includes('<th rowspan="2">姓名</th>'));
assert.equal((classPrintedHtml.match(/<th>地點<\/th>/g) || []).length, 4);
assert.ok(classPrintedHtml.includes('開始：115/9/9（三）'));
assert.ok(classPrintedHtml.includes('音樂教室'));
assert.ok(classPrintedHtml.includes('colspan="3">星期三'));
assert.ok(classPrintedHtml.includes('colspan="3">星期五'));
assert.equal((classPrintedHtml.match(/class="sheet class-sheet"/g) || []).length, 2);
assert.equal((classPrintedHtml.match(/class="class-summary-table"/g) || []).length, 2);
assert.ok(classPrintedHtml.includes('data-sheet-filter'));
assert.ok(classPrintedHtml.includes('data-print-current'));
assert.ok(classPrintedHtml.includes('data-print-all'));
assert.ok(classPrintedHtml.includes('只列印目前班級'));
assert.ok(classPrintedHtml.includes('列印全部班級'));
...
assert.ok(adminJs.includes("querySelector('[data-print-current]')"));
assert.ok(adminJs.includes("querySelector('[data-print-all]')"));
assert.ok(adminJs.includes("classList.remove('filtered-out')"));
