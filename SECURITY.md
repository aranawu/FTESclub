...
- `ADMIN_PASSWORD`、`ADMIN_SESSION_SECRET`、`ID_HASH_PEPPER`、`RESEND_API_KEY` 必須設為 Cloudflare 的加密 Secret，絕對不可提交至 GitHub。
- 管理者帳號密碼只傳給後端驗證；登入成功後，瀏覽器分頁僅暫存有效 8 小時的 HMAC 簽章憑證。
- 若管理密碼、登入簽章密鑰或郵件金鑰外洩，請立即在 Cloudflare／Resend 撤銷並更換。
